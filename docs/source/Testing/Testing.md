...
3.  **`simulate-apps`**:
    *   **Purpose**: Runs Verilator RTL simulations for all applications (except the blacklisted ones) to verify their runtime behavior by batches of 6 in parrallel.
    ...
    *   **Steps**:
        *   Downloads the simulator generated by `build-simulator`.
        *   Executes `python3 test/test_apps/test_apps.py --apps "${{ matrix.apps }}" --compilers gcc --skip-build-sim` to compile and simulate all applications without building the sim.
...
8.  **`build-simulator`**:
    *   **Purpose**: Builds all the simulators needed for the the simulations.
    *   **Dependencies**: Depends on `lint`.
    *   **Environment**: Runs inside the `x-heep-toolchain` Docker container.
    *   **Steps**:
        *   Runs '`make ${{ matrix.simulator }}-build`'
        *   Uploads artifact of the simulator to github
